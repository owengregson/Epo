...
/** Design-default follow-back window (settings `maxWaitForFollowbackDays` = 4). */
const FOLLOWBACK_WINDOW_MS = 4 * DAY_MS;

/** Design-default post-followback hold (settings `holdAfterFollowbackDays` = 2). */
 ...
 */
function deriveContext(stage: QueueStageKey, row: QueueRow, now: number): RowContext {
  switch (stage) {
    case 'queued':
    ...
    case 'awaiting': {
      ...
      return {
        ...
        barFrac: clamp(elapsed / FOLLOWBACK_WINDOW_MS, 0, 1),
        brass: false,
      };
    }
    ...
    case 'held': {
      ...
      return {
        ...
        barFrac: clamp(1 - remaining / HOLD_MS, 0, 1),
        brass: false,
      };
    }
 ...
 */
export function QueueRowItem({ stage, row }: QueueRowItemProps): h.JSX.Element {
  const due = stage === 'due';
  const { ctx, barFrac, brass } = deriveContext(stage, row, Date.now());
